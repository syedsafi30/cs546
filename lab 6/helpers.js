...
const checkreview = async(movieId)=>{
    if (!movieId) throw new Error("You must provide an id to search for");
...
const postreviewcheck = async(reviewTitle, reviewerName, review, rating)=>{
    if (!reviewTitle) throw new Error("You must provide an id to search for");
    if (!reviewerName) throw new Error("You must provide an id to search for");
    if (!review) throw new Error("You must provide an id to search for");
    if (!rating) throw new Error("You must provide an id to search for");
...
const routescheck = async(movieId,) =>{
    
        if (!movieId) {
          throw "Error: You must provide an id to search for";
        }
        if (/^-?\d+$/.test(movieId) == false)
          throw "Error: string must contain only numbers";
        if (typeof movieId !== "string") throw "Error: id must be a string";
        movieId = movieId.trim();
        if (movieId.length === 0)
          throw "Error: id cannot be an empty string or just spaces";
    
        if (typeof movieId === "boolean") throw "Error: id must be a string";
    
        if (movieId === null) throw "Error: id cannot be null";
        if (movieId === NaN) throw "Error: id cannot be NaN";
    
        return movieId;
      }


const routescheckID = async(reviewId) =>{
    
    if (!reviewId) {
      throw "Error: You must provide an id to search for";
    }
    if (/^-?\d+$/.test(reviewId) == false)
      throw "Error: string must contain only numbers";
    if (typeof reviewId !== "string") throw "Error: id must be a string";
    reviewId = reviewId.trim();
    if (reviewId.length === 0)
      throw "Error: id cannot be an empty string or just spaces";

    if (typeof reviewId === "boolean") throw "Error: id must be a string";

    if (reviewId === null) throw "Error: id cannot be null";
    if (reviewId === NaN) throw "Error: id cannot be NaN";
